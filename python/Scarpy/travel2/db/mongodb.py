...
class Mongo(object):
    ...
    def find_scenics(self, city_name, count=20):
        # cursor = self.collection.find({"$or":[{'address': {'$regex':city_name},'city':{'$regex':city_name}}]}, limit=count).\
        #     sort([('comments_num', pymongo.DESCENDING)])
        #{$or:[{"city":"西安"},{"address": "西安"}]}
        cursor = self.collection.find({"$or": [{'address': {'$regex': city_name}},{ 'city': {'$regex': city_name}}]},
                                      limit=count).\
            sort([('comments_num', pymongo.DESCENDING)])
        #db.scenic.find({$or:[{"city":"西安"},{"address": "西安"}]})
        # ({"$or":[{"name":"yaya"},{"age":28}]}
        scenics = []
