...
from travel2.db.mongodb import mongo
...
class MaFengWoSpider(object):
    ...
    def __init__(self, city):
        ...
        self.url_pattern ='http://www.mafengwo.cn/search/q.php?q='+city+'&p={}&t=notes&kt=1'
        # 准备请求头
        self.headers = {
    ...
    def get_datas_from_page(self, page):
        ...
        # 1. 获取包含景点信息的标签列表
        # xpath: 返回一个list对象
        lis = element.xpath('//div/div/div/div/div/ul/li')
        ...
        for li in lis:
            ...
            item = {}
            #获取标题
            title = ''.join(li.xpath('.//h3//text()'))
            title = re.sub(r"\s+", "", title)
            title = re.sub('[\r\n\f]{2,}','\n',title)
            print(title)

            # 获取名称
            #name = ''.join(li.xpath('./a//text()'))
            # print(name)
            # 如果标题中, 没有景点就过滤掉
            # if name.find('景点') == -1:
            ...
            # # print(item)
            # # 提取地址
            # item['address'] = li.xpath('../ul/li[1]/a//text()')[0]
            # # 获取点评数量
            # # 点评(489)
            # comments_num = li.xpath('../ul/li[2]/a//text()')[0]
            # # 提取点评中的数据
            # item['comments_num'] = int(re.findall('蜂评\((\d+)\)', comments_num)[0])
            #
            # # 获取游记数量
            # # 游记(23)
            # travel_notes_num = li.xpath('../ul/li[3]/a//text()')[0]
